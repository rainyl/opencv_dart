...
CVD_TYPEDEF(void *, FaceRecognizerSF)
    ...
    CvStatus CascadeClassifier_DetectMultiScaleWithParams(CascadeClassifier self, Mat img, VecRect *objects, double scale, int minNeighbors, int flags, Size minSize, Size maxSize);
    CvStatus CascadeClassifier_DetectMultiScale2(CascadeClassifier self, Mat img, VecRect *objects, VecInt *numDetections, double scaleFactor, int minNeighbors, int flags, Size minSize, Size maxSize);
    CvStatus CascadeClassifier_DetectMultiScale3(CascadeClassifier self, Mat img, VecRect *objects, VecInt *rejectLevels, VecDouble *levelWeights, double scaleFactor, int minNeighbors, int flags, Size minSize, Size maxSize, bool outputRejectLevels);
    ...
    CvStatus HOGDescriptor_Detect(HOGDescriptor self, Mat img, VecPoint *foundLocations, VecDouble *weights, double hitThresh, Size winStride, Size padding, VecPoint *searchLocations);
    CvStatus HOGDescriptor_Detect2(HOGDescriptor self, Mat img, VecPoint *foundLocations, double hitThresh, Size winStride, Size padding, VecPoint *searchLocations);
    CvStatus HOGDescriptor_DetectMultiScale(HOGDescriptor self, Mat img, VecRect *rval);
    CvStatus HOGDescriptor_DetectMultiScaleWithParams(HOGDescriptor self, Mat img, double hitThresh, Size winStride, Size padding, double scale, double finalThreshold, bool useMeanshiftGrouping, VecRect *rval);
    CvStatus HOG_GetDefaultPeopleDetector(VecFloat *rval);
    CvStatus HOGDescriptor_SetSVMDetector(HOGDescriptor self, VecFloat det);
    CvStatus HOGDescriptor_Compute(HOGDescriptor self, Mat img, VecFloat *descriptors, Size winStride, Size padding, VecPoint *locations);
    CvStatus HOGDescriptor_computeGradient(HOGDescriptor self, Mat img, Mat grad, Mat angleOfs, Size paddingTL, Size paddingBR);
    ...
    CvStatus HOGDescriptor_groupRectangles(HOGDescriptor self, VecRect rectList, VecDouble weights, int groupThreshold, double eps);
    ...
    CvStatus QRCodeDetector_DetectAndDecode(QRCodeDetector self, Mat input, VecPoint *points, Mat *straight_qrcode, char **rval);
    CvStatus QRCodeDetector_Detect(QRCodeDetector self, Mat input, VecPoint *points, bool *rval);
    CvStatus QRCodeDetector_Decode(QRCodeDetector self, Mat input, VecPoint *points, Mat straight_qrcode, char **rval);
    CvStatus QRCodeDetector_decodeCurved(QRCodeDetector self, Mat img, VecPoint points, CVD_OUT Mat *straight_qrcode, char **rval);
    CvStatus QRCodeDetector_detectAndDecodeCurved(QRCodeDetector self, Mat img, VecPoint *points, CVD_OUT Mat *straight_qrcode, char **rval);
    CvStatus QRCodeDetector_DetectMulti(QRCodeDetector self, Mat input, VecPoint *points, bool *rval);
    CvStatus QRCodeDetector_DetectAndDecodeMulti(QRCodeDetector self, Mat input, VecVecChar *decoded, VecPoint *points, VecMat *straight_code, bool *rval);
    ...
    // FaceDetectorYN
    CvStatus FaceDetectorYN_New(const char *model, const char *config, Size input_size, float score_threshold, float nms_threshold, int top_k, int backend_id, int target_id, FaceDetectorYN *rval);
    CvStatus FaceDetectorYN_NewFromBuffer(const char *framework, VecUChar buffer, VecUChar buffer_config, Size input_size, float score_threshold, float nms_threshold, int top_k, int backend_id, int target_id, FaceDetectorYN *rval);
    ...
    // FaceRecognizerSF
    CvStatus FaceRecognizerSF_New(const char *model, const char *config, int backend_id, int target_id, FaceRecognizerSF *rval);
