...
inline std::vector<int> vecint_c2cpp(VecInt v) {
  std::vector<int> rv;
  for (int i = 0; i < v.length; i++) {
    int p = v.ptr[i];
    rv.push_back(p);
...
inline VecInt vecint_cpp2c(std::vector<int> v) {
  int *ptr = new int[v.size()];
  for (int i = 0; i < v.size(); i++) { ptr[i] = v[i]; }
  return VecInt{.ptr = ptr, .length = v.size()};
}

inline std::vector<float> vecfloat_c2cpp(VecFloat v) {
  std::vector<float> rv;
  for (int i = 0; i < v.length; i++) {
    float p = v.ptr[i];
    rv.push_back(p);
...
inline VecFloat vecfloat_cpp2c(std::vector<float> v) {
  float *ptr = new float[v.size()];
  for (int i = 0; i < v.size(); i++) { ptr[i] = v[i]; }
  return VecFloat{.ptr = ptr, .length = v.size()};
}

inline std::vector<double> vecdouble_c2cpp(VecDouble v) {
...
inline VecDouble vecdouble_cpp2c(std::vector<double> v) {
  double *ptr = new double[v.size()];
  for (int i = 0; i < v.size(); i++) { ptr[i] = v[i]; }
  return VecDouble{.ptr = ptr, .length = v.size()};
}
...
inline VecKeyPoint veckeypoint_cpp2c(std::vector<cv::KeyPoint> v) {
  ...
  for (int i = 0; i < v.size(); i++) {
    ptr[i] = {
        v[i].pt.x, v[i].pt.y, v[i].size, v[i].angle, v[i].response, v[i].octave, v[i].class_id
    };
  }
