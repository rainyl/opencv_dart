...
CvStatus *VecPoint_NewFromPointer(Point *points, int length, VecPoint *rval) {
  ...
  for (int i = 0; i < length; i++) { vec.push_back(cv::Point(points[i].x, points[i].y)); }
...
CvStatus *VecPoint2f_NewFromPointer(Point2f *points, int length, VecPoint2f *rval) {
  ...
  for (int i = 0; i < length; i++) { vec.push_back(cv::Point2f(points[i].x, points[i].y)); }
...
CvStatus *VecUChar_NewFromPointer(uchar *p, int length, VecUChar *rval) {
  BEGIN_WRAP
  std::vector<uchar> v;
  for (int i = 0; i < length; i++) { v.push_back(p[i]); }
...
CvStatus *VecMat_NewFromPointer(Mat *mats, int length, VecMat *rval) {
  BEGIN_WRAP
  std::vector<cv::Mat> v;
  for (int i = 0; i < length; i++) { v.push_back(*(mats[i].ptr)); }
...
CvStatus *VecVec4f_New(VecVec4f *rval) {
  BEGIN_WRAP
  *rval = {new std::vector<cv::Vec4f>()};
  END_WRAP
}
CvStatus *VecVec4f_NewFromPointer(Vec4f *data, int length, VecVec4f *rval) {
  BEGIN_WRAP
  std::vector<cv::Vec4f> vec;
  for (int i = 0; i < length; i++) {
    Vec4f v = data[i];
    vec.push_back(cv::Vec4f(v.val1, v.val2, v.val3, v.val4));
  }
  *rval = {new std::vector<cv::Vec4f>(vec)};
  END_WRAP
}
// CvStatus VecVec4f_NewFromVec(VecVec4f vec, VecVec4f *rval);
CvStatus *VecVec4f_At(VecVec4f vec, int idx, Vec4f *rval) {
  BEGIN_WRAP
  cv::Vec4f v = vec.ptr->at(idx);
  *rval = {v.val[0], v.val[1], v.val[2], v.val[3]};
  END_WRAP
}
// CvStatus VecVec4f_Data(VecVec4f vec, Vec4f **rval){}
CvStatus *VecVec4f_Append(VecVec4f vec, Vec4f v) {
  BEGIN_WRAP
  vec.ptr->push_back(cv::Vec4f(v.val1, v.val2, v.val3, v.val4));
  END_WRAP
}
CvStatus *VecVec4f_Size(VecVec4f vec, int *rval) {
  BEGIN_WRAP
  *rval = static_cast<int>(vec.ptr->size());
  END_WRAP
}
void VecVec4f_Close(VecVec4fPtr vec) {
  vec->ptr->clear();
  CVD_FREE(vec);
}

CvStatus *VecVec6f_New(VecVec6f *rval) {
  BEGIN_WRAP
  *rval = {new std::vector<cv::Vec6f>()};
  END_WRAP
}
CvStatus *VecVec6f_NewFromPointer(Vec6f *data, int length, VecVec6f *rval) {
  ...
  for (int i = 0; i < length; i++) {
    Vec6f v = data[i];
    vec.push_back(cv::Vec6f(v.val1, v.val2, v.val3, v.val4, v.val5, v.val6));
  }
  *rval = {new std::vector<cv::Vec6f>(vec)};
  END_WRAP
}
// CvStatus VecVec6f_NewFromVec(VecVec6f vec, VecVec6f *rval);
CvStatus *VecVec6f_At(VecVec6f vec, int idx, Vec6f *rval) {
  BEGIN_WRAP
  cv::Vec6f v = vec.ptr->at(idx);
  *rval = {v.val[0], v.val[1], v.val[2], v.val[3], v.val[4], v.val[5]};
  END_WRAP
}
// CvStatus VecVec6f_Data(VecVec6f vec, Vec6f **rval);
CvStatus *VecVec6f_Append(VecVec6f vec, Vec6f v) {
  BEGIN_WRAP
  vec.ptr->push_back(cv::Vec6f(v.val1, v.val2, v.val3, v.val4, v.val5, v.val6));
  END_WRAP
}
CvStatus *VecVec6f_Size(VecVec6f vec, int *rval) {
  BEGIN_WRAP
  *rval = static_cast<int>(vec.ptr->size());
  END_WRAP
}
void VecVec6f_Close(VecVec6fPtr vec) {
  vec->ptr->clear();
  CVD_FREE(vec);
}
