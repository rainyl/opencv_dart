...
class IosCMakeConan(ConanFile):
    ...
    exports_sources = "cmake_wrapper"

    @property
    def _source_subfolder(self):
        return os.path.join(self.source_folder, "source_subfolder")  # type: ignore
    ...
    def configure(self):
        if not apple.is_apple_os(self):
            raise ConanInvalidConfiguration(
                "This package only supports Apple operating systems"
            )
    ...
    def source(self):
        cfiles.get(self, **self.conan_data["sources"][self.version])  # type: ignore
        fname = f"ios-cmake-{self.version}"
        os.rename(fname, self._source_subfolder)
        print(f"{fname}->{self._source_subfolder}")

    def build(self):
        pass  # there is nothing to build

    def package(self):
        print(f"{self.package_folder}")
        cfiles.copy(self, "cmake_wrapper", ".", dst="bin")
        cfiles.copy(
            self,
            "ios.toolchain.cmake",
            src=self._source_subfolder,
            dst="lib/cmake/ios-cmake",
            keep_path=False,
        )
        self._chmod_plus_x("bin/cmake_wrapper")
        ...
        cfiles.save(self, f"{self.package_folder}/include/dummy_header.h", "\n")
    ...
    def package_info(self):
        ...
        cmake_wrapper = os.path.join(f"{self.package_folder}", "bin", "cmake_wrapper")
        ...
        tool_chain = os.path.join(
            f"{self.package_folder}", "lib", "cmake", "ios-cmake", "ios.toolchain.cmake"
        )
