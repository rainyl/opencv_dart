...
class ximgproc_rl {
  ...
  static Mat dilate(InputArray rlSrc, InputArray rlKernel, {(int, int) anchor = (0, 0)}) {
    final p = calloc<cvg.Mat>();
    cvRun(() => ccontrib.ximgproc_rl_dilate(rlSrc.ref, p, rlKernel.ref, anchor.toPoint().ref));
    return Mat.fromPointer(p);
  }
  ...
  static Future<Mat> dilateAsync(InputArray rlSrc, InputArray rlKernel, {(int, int) anchor = (0, 0)}) async =>
      cvRunAsync(
        (callback) =>
            ccontrib.ximgproc_rl_dilate_Async(rlSrc.ref, rlKernel.ref, anchor.toPoint().ref, callback),
        matCompleter,
      );
  ...
  static Mat erode(
    ...
    final p = calloc<cvg.Mat>();
    cvRun(() => ccontrib.ximgproc_rl_erode(rlSrc.ref, p, rlKernel.ref, bBoundaryOn, anchor.toPoint().ref));
    return Mat.fromPointer(p);
  }
