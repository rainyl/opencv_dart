...
CvStatus *
cv_VideoCapture_create_2(int index, int apiPreference, VideoCapture *rval, CvCallback_0 callback) {
  BEGIN_WRAP
  rval->ptr = new cv::VideoCapture(index, apiPreference);
  if (callback != nullptr) { callback(); }
  END_WRAP
}
void cv_VideoCapture_close(VideoCapturePtr self) { CVD_FREE(self); }

CvStatus *
cv_VideoCapture_open(VideoCapture self, const char *uri, bool *rval, CvCallback_0 callback) {
  BEGIN_WRAP
...
void cv_VideoCapture_set(VideoCapture self, int prop, double val) { self.ptr->set(prop, val); }
double cv_VideoCapture_get(VideoCapture self, int prop) { return self.ptr->get(prop); }
bool cv_VideoCapture_isOpened(VideoCapture self) { return self.ptr->isOpened(); }
...
CvStatus *cv_video_registry_getBackends(int **rval, int *size) {
  BEGIN_WRAP
  auto v = cv::videoio_registry::getBackends();
  auto p = static_cast<int *>(calloc(v.size(), sizeof(int)));
  for (int i = 0; i < v.size(); i++) { p[i] = static_cast<int>(v[i]); }
  *rval = p;
  *size = v.size();
  END_WRAP
}
...
CvStatus *cv_video_registry_getCameraBackends(int **rval, int *size) {
  BEGIN_WRAP
  auto v = cv::videoio_registry::getCameraBackends();
  auto p = static_cast<int *>(calloc(v.size(), sizeof(int)));
  for (int i = 0; i < v.size(); i++) { p[i] = static_cast<int>(v[i]); }
  *rval = p;
  *size = v.size();
...
CvStatus *cv_video_registry_getStreamBackends(int **rval, int *size) {
  BEGIN_WRAP
  auto v = cv::videoio_registry::getStreamBackends();
  auto p = static_cast<int *>(calloc(v.size(), sizeof(int)));
  for (int i = 0; i < v.size(); i++) { p[i] = static_cast<int>(v[i]); }
  *rval = p;
  *size = v.size();
...
CvStatus *cv_video_registry_getStreamBufferedBackends(int **rval, int *size) {
  BEGIN_WRAP
  auto v = cv::videoio_registry::getStreamBufferedBackends();
  auto p = static_cast<int *>(calloc(v.size(), sizeof(int)));
  for (int i = 0; i < v.size(); i++) { p[i] = static_cast<int>(v[i]); }
  *rval = p;
  *size = v.size();
...
CvStatus *cv_video_registry_getWriterBackends(int **rval, int *size) {
  BEGIN_WRAP
  auto v = cv::videoio_registry::getWriterBackends();
  auto p = static_cast<int *>(calloc(v.size(), sizeof(int)));
  for (int i = 0; i < v.size(); i++) { p[i] = static_cast<int>(v[i]); }
  *rval = p;
  *size = v.size();
...
void cv_VideoWriter_close(VideoWriterPtr self) { CVD_FREE(self); }
...
bool cv_VideoWriter_isOpened(VideoWriter self) { return self.ptr->isOpened(); }
...
double cv_VideoWriter_get(VideoWriter self, int propId) { return self.ptr->get(propId); }

void cv_VideoWriter_set(VideoWriter self, int propId, double val) { self.ptr->set(propId, val); }
