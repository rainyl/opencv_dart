...
class Mat extends CvStruct<cvg.Mat> {
  ...
  factory Mat.fromScalar(int rows, int cols, MatType type, Scalar s) {
    final p = calloc<cvg.Mat>();
    cvRun(() => cffi.Mat_NewFromScalar(s.ref, rows, cols, type.value, p));
    final mat = Mat._(p);
    return mat;
  }
  ...
  //SECTION - Properties
  MatType get type => MatType(cffi.Mat_Type(ref));

  int get flags => cffi.Mat_Flags(ref);

  int get width => cols;
  int get height => rows;
  int get cols => cffi.Mat_Cols(ref);
  int get rows => cffi.Mat_Rows(ref);
  int get channels => cffi.Mat_Channels(ref);
  int get total => cffi.Mat_Total(ref);
  bool get isEmpty => cffi.Mat_Empty(ref);
  bool get isContinus => cffi.Mat_IsContinuous(ref);
  bool get isSubmatrix => cffi.Mat_IsSubmatrix(ref);
  (int, int, int) get step {
    final ms = cffi.Mat_Step(ref);
    return (ms.p[0], ms.p[1], ms.p[2]);
  }

  int get elemSize => cffi.Mat_ElemSize(ref);
  int get elemSize1 => cffi.Mat_ElemSize1(ref);
  int get dims => cffi.Mat_Dims(ref);
  ...
  ffi.Pointer<U8> get dataPtr => cffi.Mat_Data(ref).cast<U8>();

  /// Mat.size
  VecI32 get size => VecI32.fromPointer(cffi.Mat_Size(ref));
  ...
  int get countNoneZero {
    ...
    return cffi.Mat_CountNonZero(ref);
  }
  ...
  Mat subtractMat(Mat other, {bool inplace = false}) =>
      _opMat(other, cffi.Mat_SubtractMat, inplace: inplace);

  Mat subtractU8(int val, {bool inplace = false}) {
    assert(type.depth == MatType.CV_8U && val >= CV_U8_MIN && val <= CV_U8_MAX);
    return _opInt(val, cffi.Mat_SubtractUChar, inplace: inplace);
  }

  Mat subtractI8(int val, {bool inplace = false}) {
    assert(type.depth == MatType.CV_8S && val >= CV_I8_MIN && val <= CV_I8_MAX);
    return _opInt(val, cffi.Mat_SubtractSChar, inplace: inplace);
  }

  Mat subtractI32(int val, {bool inplace = false}) {
    assert(type.depth == MatType.CV_32S && val >= CV_I32_MIN && val <= CV_I32_MAX);
    return _opInt(val, cffi.Mat_SubtractI32, inplace: inplace);
  }

  Mat subtractI16(int val, {bool inplace = false}) {
    assert(type.depth == MatType.CV_16S && val >= CV_I16_MIN && val <= CV_I16_MAX);
    return _opInt(val, cffi.Mat_SubtractI16, inplace: inplace);
  }

  Mat subtractU16(int val, {bool inplace = false}) {
    assert(type.depth == MatType.CV_16U && val >= CV_U16_MIN && val <= CV_U16_MAX);
    return _opInt(val, cffi.Mat_SubtractU16, inplace: inplace);
  }

  Mat subtractF32(double val, {bool inplace = false}) {
    assert(type.depth == MatType.CV_32F && val <= CV_F32_MAX);
    return _opDouble(val, cffi.Mat_SubtractFloat, inplace: inplace);
  }

  Mat subtractF64(double val, {bool inplace = false}) {
    assert(type.depth == MatType.CV_64F && val <= CV_F64_MAX);
    return _opDouble(val, cffi.Mat_SubtractF64, inplace: inplace);
  }
  ...
  Mat multiplyMat(Mat other, {bool inplace = false}) =>
      _opMat(other, cffi.Mat_MultiplyMat, inplace: inplace);

  Mat multiplyU8(int val, {bool inplace = false}) {
    assert(type.depth == MatType.CV_8U && val >= CV_U8_MIN && val <= CV_U8_MAX);
    return _opInt(val, cffi.Mat_MultiplyUChar, inplace: inplace);
  }

  Mat multiplyI8(int val, {bool inplace = false}) {
    assert(type.depth == MatType.CV_8S && val >= CV_I8_MIN && val <= CV_I8_MAX);
    return _opInt(val, cffi.Mat_MultiplySChar, inplace: inplace);
  }

  Mat multiplyI16(int val, {bool inplace = false}) {
    assert(type.depth == MatType.CV_16S && val >= CV_I16_MIN && val <= CV_I16_MAX);
    return _opInt(val, cffi.Mat_MultiplyI16, inplace: inplace);
  }

  Mat multiplyU16(int val, {bool inplace = false}) {
    assert(type.depth == MatType.CV_16U && val >= CV_U16_MIN && val <= CV_U16_MAX);
    return _opInt(val, cffi.Mat_MultiplyU16, inplace: inplace);
  }

  Mat multiplyI32(int val, {bool inplace = false}) {
    assert(type.depth == MatType.CV_32S && val >= CV_I32_MIN && val <= CV_I32_MAX);
    return _opInt(val, cffi.Mat_MultiplyI32, inplace: inplace);
  }

  Mat multiplyF32(double val, {bool inplace = false}) {
    assert(type.depth == MatType.CV_32F && val <= CV_F32_MAX);
    return _opDouble(val, cffi.Mat_MultiplyFloat, inplace: inplace);
  }

  Mat multiplyF64(double val, {bool inplace = false}) {
    assert(type.depth == MatType.CV_64F && val <= CV_F64_MAX);
    return _opDouble(val, cffi.Mat_MultiplyF64, inplace: inplace);
  }
  ...
  Mat divideMat(Mat other, {bool inplace = false}) => _opMat(other, cffi.Mat_DivideMat, inplace: inplace);

  Mat divideU8(int val, {bool inplace = false}) {
    assert(type.depth == MatType.CV_8U && val >= CV_U8_MIN && val <= CV_U8_MAX);
    return _opInt(val, cffi.Mat_DivideUChar, inplace: inplace);
  }

  Mat divideI8(int val, {bool inplace = false}) {
    assert(type.depth == MatType.CV_8S && val >= CV_I8_MIN && val <= CV_I8_MAX);
    return _opInt(val, cffi.Mat_DivideSChar, inplace: inplace);
  }

  Mat divideI16(int val, {bool inplace = false}) {
    assert(type.depth == MatType.CV_16S && val >= CV_I16_MIN && val <= CV_I16_MAX);
    return _opInt(val, cffi.Mat_DivideI16, inplace: inplace);
  }

  Mat divideU16(int val, {bool inplace = false}) {
    assert(type.depth == MatType.CV_16U && val >= CV_U16_MIN && val <= CV_U16_MAX);
    return _opInt(val, cffi.Mat_DivideU16, inplace: inplace);
  }

  Mat divideI32(int val, {bool inplace = false}) {
    assert(type.depth == MatType.CV_32S && val >= CV_I32_MIN && val <= CV_I32_MAX);
    return _opInt(val, cffi.Mat_DivideI32, inplace: inplace);
  }

  Mat divideF32(double val, {bool inplace = false}) {
    assert(type.depth == MatType.CV_32F && val <= CV_F32_MAX);
    return _opDouble(val, cffi.Mat_DivideFloat, inplace: inplace);
  }

  Mat divideF64(double val, {bool inplace = false}) {
    assert(type.depth == MatType.CV_64F && val <= CV_F64_MAX);
    return _opDouble(val, cffi.Mat_DivideF64, inplace: inplace);
  }
  ...
  Mat adjustROI(int dtop, int dbottom, int dleft, int dright) {
    final p = calloc<cvg.Mat>();
    cvRun(() => cffi.Mat_AdjustROI(ref, dtop, dbottom, dleft, dright, p));
    return Mat._(p);
  }
  ...
  (Size wholeSize, Point ofs) locateROI() {
    ...
    cvRun(() => cffi.Mat_LocateROI(ref, pWholeSize, pOfs));
  ...
  Mat mul(Mat other, {bool inplace = false, double scale = 1.0}) {
    ...
    cvRun(() => cffi.Mat_Mul(ref, other.ref, p, scale));
  ...
  Mat setTo(Scalar value, {Mat? mask}) {
    mask ??= Mat.empty();
    cvRun(() => cffi.Mat_SetTo(ref, value.ref, mask!.ref));
    return this;
  }
  ...
  String toFmtString({
    ...
    cvRun(() => cffi.Mat_toString(ref, fmtType, f16Precision, f32Precision, f64Precision, multiLine, p));
