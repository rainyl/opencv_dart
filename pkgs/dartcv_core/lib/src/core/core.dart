...
void setLogLevel(int logLevel) {
  cvRun(() => cffi.setLogLevel(logLevel));
}

/// Gets the global logging level.
int getLogLevel() {
  final p = calloc<ffi.Int>();
  cvRun(() => cffi.getLogLevel(p));
  final level = p.value;
  calloc.free(p);
  return level;
}
...
Mat add(Mat src1, Mat src2, {Mat? dst, int dtype = -1, Mat? mask}) {
  dst ??= Mat.empty();
  mask ??= Mat.empty();
  cvRun(() => cffi.Mat_Add(src1.ref, src2.ref, dst!.ref, mask!.ref, dtype));
  return dst;
}
...
Mat addWeighted(
  ...
  dst ??= Mat.empty();
  cvRun(() => cffi.Mat_AddWeighted(src1.ref, alpha, src2.ref, beta, gamma, dst!.ref, dtype));
  return dst;
}
...
(Mat magnitude, Mat angle) cartToPolar(
  ...
  magnitude ??= Mat.empty();
  angle ??= Mat.empty();
  cvRun(() => cffi.Mat_CartToPolar(x.ref, y.ref, magnitude!.ref, angle!.ref, angleInDegrees));
...
int countNonZero(Mat src) => cffi.Mat_CountNonZero(src.ref);
...
Mat dft(InputArray src, {OutputArray? dst, int flags = 0, int nonzeroRows = 0}) {
  dst ??= Mat.empty();
  cvRun(() => cffi.Mat_DFT(src.ref, dst!.ref, flags, nonzeroRows));
  return dst;
}
...
Mat divide(InputArray src1, InputArray src2, {OutputArray? dst, double scale = 1, int dtype = -1}) {
  dst ??= Mat.empty();
  cvRun(() => cffi.Mat_Divide(src1.ref, src2.ref, dst!.ref, scale, dtype));
  return dst;
}
...
Mat mulSpectrums(
  ...
  c ??= Mat.empty();
  cvRun(() => cffi.Mat_MulSpectrums(a.ref, b.ref, c!.ref, flags, conjB));
  return c;
}
...
Mat multiply(
  ...
  dst ??= Mat.empty();
  cvRun(() => cffi.Mat_Multiply(src1.ref, src2.ref, dst!.ref, scale, dtype));
  return dst;
}
...
Mat normalize(
  ...
  mask ??= Mat.empty();
  cvRun(() => cffi.Mat_Normalize(src.ref, dst.ref, alpha, beta, normType, dtype, mask!.ref));
  return dst;
}
...
double norm(
  ...
  cvRun(() => cffi.Norm(src1.ref, normType, mask!.ref, p));
  final rval = p.value;
  calloc.free(p);
  return rval;
}
...
double norm1(
  ...
  return cvRunArena<double>((arena) {
    final p = arena<ffi.Double>();
    mask ??= Mat.empty();
    cvRun(() => cffi.NormWithMats(src1.ref, src2.ref, normType, mask!.ref, p));
    return p.value;
  });
}
...
Mat subtract(
  ...
  dst ??= Mat.empty();
  mask ??= Mat.empty();
  cvRun(() => cffi.Mat_Subtract(src1.ref, src2.ref, dst!.ref, mask!.ref, dtype));
  return dst;
}
