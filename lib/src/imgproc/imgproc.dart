...
List<Point> approxPolyDP(VecPoint curve, double epsilon, bool closed) {
  return cvRunArena<List<Point>>((arena) {
    final vec = VecPoint.fromList([]);
    cvRun(() => CFFI.ApproxPolyDP(curve.ref, epsilon, closed, vec.ptr));
    return vec.toList();
  });
}
...
//   CFFI.ConvexHull(pointVec, hull.ref, clockwise, returnPoints);
//   CFFI.PointVector_Close(pointVec);
// }
...
//   CFFI.ConvexityDefects(pointVec, hull.ref, result.ref);
//   CFFI.PointVector_Close(pointVec);
// }
...
//   CFFI.CvtColor(src.ref, dst.ref, code);
// }
...
//   CFFI.EqualizeHist(src.ref, dst.ref);
// }
...
//   final result = CFFI.CompareHist(hist1.ref, hist2.ref, method);
//   return result;
// }
...
//       ..ref.x = pt1.$1
//       ..ref.y = pt1.$2;
//     r = CFFI.ClipLine(size.ref, cPt1.ref, cPt2.ref);
//   });
//   return r;
// }
...
//   CFFI.BilateralFilter(src.ref, dst.ref, diameter, sigmaColor, sigmaSpace);
// }
...
// ) {
//   using((arena) {
//     CFFI.Blur(src.ref, dst.ref, ksize.toSize(arena).ref);
//   });
// }
...
//   using((arena) {
//     CFFI.BoxFilter(src.ref, dst.ref, depth, ksize.toSize(arena).ref);
//   });
// }
...
//   using((arena) {
//     CFFI.SqBoxFilter(src.ref, dst.ref, depth, ksize.toSize(arena).ref);
//   });
// }
...
//   final pointVec = points.toNativeVecotr();
//   final rect = Rect.fromNative(CFFI.BoundingRect(pointVec));
//   CFFI.PointVector_Close(pointVec);
//   return rect;
// }
...
//   CFFI.BoxPoints(rect.ref, pts.ref);
// }
...
//   final pointVec = contour.toNativeVecotr();
//   final area = CFFI.ContourArea(contour.toNativeVecotr());
//   CFFI.PointVector_Close(pointVec);
//   return area;
// }
...
//   final pointVec = points.toNativeVecotr();
//   final r = CFFI.MinAreaRect(pointVec);
...
//   final r = CFFI.FitEllipse(pointVec);
//   CFFI.PointVector_Close(pointVec);
...
//     CFFI.MinEnclosingCircle(pointVec, _center, _radius);
//     CFFI.PointVector_Close(pointVec);
...
(Contours contours, Mat hierarchy) findContours(Mat src, int mode, int method) {
  ...
  final vec = VecVecPoint.fromList([]);
  cvRun(() => CFFI.FindContours(src.ref, hierarchy.ref, mode, method, vec.ptr));
...
//   final pointVec = pts.toNativeVecotr();
//   final r = CFFI.PointPolygonTest(pointVec, pt.toNative(), measureDist);
//   CFFI.PointVector_Close(pointVec);
//   return r;
// }
...
//   int ccltype,
// ) {
//   return CFFI.ConnectedComponents(
//     image.ref,
//     labels.ref,
//     connectivity,
//     ltype,
...
//   int ccltype,
// ) {
//   return CFFI.ConnectedComponentsWithStats(
//     src.ref,
//     labels.ref,
//     stats.ref,
//     centroids.ref,
...
//   mask = mask ?? Mat.empty();
//   CFFI.MatchTemplate(image.ref, templ.ref, result.ref, method, mask.ref);
// }
...
//   final m = CFFI.Moments(src.ref, binaryImage);
...
// }) {
//   using((arena) {
//     CFFI.PyrDown(src.ref, dst.ref, dstsize.toSize(arena).ref, borderType);
//   });
// }
...
// }) {
//   using((arena) {
//     CFFI.PyrUp(src.ref, dst.ref, dstsize.toSize(arena).ref, borderType);
//   });
// }
...
//   return CFFI.MorphologyDefaultBorderValue();
// }
...
//   using((p0) {
//     final r = CFFI.GetStructuringElement(shape, ksize.toSize(p0).ref);
...
//   final r = CFFI.GetGaussianKernel(ksize, sigma, ktype);
...
//   int borderType = BORDER_DEFAULT,
// }) {
//   CFFI.SpatialGradient(src.ref, dx.ref, dy.ref, ksize, borderType);
// }
...
//   int borderType = BORDER_DEFAULT,
// }) {
//   CFFI.Scharr(src.ref, dst.ref, ddepth, dx, dy, scale, delta, borderType);
// }
...
//   CFFI.MedianBlur(src.ref, dst.ref, ksize);
// }
...
// }) {
//   CFFI.Canny(image.ref, edges.ref, threshold1, threshold2);
// }
...
// }) {
//   CFFI.HoughLines(image.ref, lines.ref, rho, theta, threshold);
// }
...
//   int sqdepth = -1,
// }) {
//   CFFI.Integral(src.ref, sum.ref, sqsum.ref, tilted.ref);
// }
...
//   int type,
// ) {
//   return CFFI.Threshold(src.ref, dst.ref, thresh, maxval, type);
// }
...
void line(
  ...
  cvRun(() => CFFI.Line(img.ref, pt1.ref, pt2.ref, color.ref, thickness));
}
...
//     offset.toNative(),
//   );
//   CFFI.PointsVector_Close(pointsVec);
// }
...
//     final _size = CFFI.GetTextSizeWithBaseline(
//       textPtr.cast(),
//       fontFace,
//       fontScale,
//       thickness,
...
//   final mat = CFFI.GetRotationMatrix2D(center.toNative(), angle, scale);
...
//   CFFI.Watershed(image.ref, markers.ref);
// }
...
//   CFFI.ApplyColorMap(src.ref, dst.ref, colormap);
// }
...
// ) {
//   CFFI.ApplyCustomColorMap(src.ref, dst.ref, userColor.ref);
// }
...
//   final mat = CFFI.GetPerspectiveTransform(srcVec, dstVec);
//   CFFI.PointVector_Close(srcVec);
//   CFFI.PointVector_Close(dstVec);
//   return Mat.fromCMat(mat);
// }
...
//   final srcVec = src.toNativeVecotr();
//   final dstVec = dst.toNativeVecotr();
//   final mat = CFFI.GetPerspectiveTransform2f(srcVec, dstVec);
//   CFFI.PointVector_Close(srcVec);
//   CFFI.PointVector_Close(dstVec);
//   return Mat.fromCMat(mat);
// }
...
//   final srcVec = src.toNativeVecotr();
//   final dstVec = dst.toNativeVecotr();
//   final mat = CFFI.GetAffineTransform(srcVec, dstVec);
//   CFFI.PointVector_Close(srcVec);
//   CFFI.PointVector_Close(dstVec);
//   return Mat.fromCMat(mat);
// }
...
//   final srcVec = src.toNativeVecotr();
//   final dstVec = dst.toNativeVecotr();
//   final mat = CFFI.GetAffineTransform2f(srcVec, dstVec);
//   CFFI.PointVector_Close(srcVec);
//   CFFI.PointVector_Close(dstVec);
//   return Mat.fromCMat(mat);
// }
...
// // }) {
// //   final mat = CFFI.FindHomography(
// //     srcPoints.ref,
// //     dstPoints.ref,
// //     method,
// //     ransacReprojThreshold,
// //     mask == null ? CFFI.Mat_New() : mask.ref,
// //     maxIters,
// //     confidence,
// //   );
// //   return Mat.fromCMat(mat);
...
//   int flags,
// ) {
//   CFFI.LinearPolar(src.ref, dst.ref, center.ref, maxRadius, flags);
// }
...
//   CFFI.FitLine(pv, line.ref, distType, param, reps, aeps);
//   CFFI.PointVector_Close(pv);
// }
...
//   final r = CFFI.MatchShapes(pv1, pv2, method, parameter);
//   CFFI.PointVector_Close(pv1);
//   CFFI.PointVector_Close(pv2);
//   return r;
// }
...
//   iM ??= Mat.empty();
//   CFFI.InvertAffineTransform(M.ref, iM.ref);
//   return iM;
// }
...
//   if (mask == null)
//     CFFI.Mat_Accumulate(src.ref, dst.ref);
//   else
//     CFFI.Mat_AccumulateWithMask(src.ref, dst.ref, mask.ref);
// }
...
//   if (mask == null)
//     CFFI.Mat_AccumulateSquare(src.ref, dst.ref);
//   else
//     CFFI.Mat_AccumulateSquareWithMask(src.ref, dst.ref, mask.ref);
// }
...
//   if (mask == null)
//     CFFI.Mat_AccumulateProduct(src1.ref, src2.ref, dst.ref);
//   else
//     CFFI.Mat_AccumulateProductWithMask(src1.ref, src2.ref, dst.ref, mask.ref);
// }
...
//   if (mask == null)
//     CFFI.Mat_AccumulatedWeighted(src.ref, dst.ref, alpha);
//   else
//     CFFI.Mat_AccumulatedWeightedWithMask(src.ref, dst.ref, alpha, mask.ref);
// }
