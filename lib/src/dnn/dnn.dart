...
class Layer extends CvStruct<cdnn.Layer> {
  ...
  static final finalizer = OcvFinalizer<cdnn.LayerPtr>(cdnn.addresses.Layer_Close);
...
class Net extends CvStruct<cdnn.Net> {
  ...
  VecMat forwardLayers(List<String> names) {
    ...
    final vecMat = calloc<cdnn.VecMat>();
  ...
  (VecF32, VecI32) getInputDetails() {
    final sc = calloc<cdnn.VecF32>();
    final zp = calloc<cdnn.VecI32>();
    cvRun(() => cdnn.Net_GetInputDetails(ref, sc, zp));
    return (VecF32.fromPointer(sc), VecI32.fromPointer(zp));
  }

  static final finalizer = OcvFinalizer<cdnn.NetPtr>(cdnn.addresses.Net_Close);
